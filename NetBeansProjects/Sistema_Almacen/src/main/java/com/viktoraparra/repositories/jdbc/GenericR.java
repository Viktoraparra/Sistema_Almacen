...
public class GenericR<E> implements I_GenericR<E>{
    ...
    public void save(E e) {
        if (e==null) return;
        //Hago la busqueda de la tabla basada en el objeto
        try {
...
//      proceso para llamar los metodos "get"
            try (PreparedStatement ps=conn.prepareStatement(
                query, PreparedStatement.RETURN_GENERATED_KEYS)){
            for (int b=1;b<campos.length; b++) {
                String method="get"
                  +campos[b].getName().substring(0,1).toUpperCase()
                  +campos[b].getName().substring(1);
                ps.setObject(b, e.getClass().getMethod(method, null).invoke(e, null));
//                System.out.println(ps);
            }
                System.out.println(ps);
            ps.execute();
            ResultSet rs=ps.getGeneratedKeys();
            //Proceso para setear la clase
            String method="set"
